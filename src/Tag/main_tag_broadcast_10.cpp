// ============================================================
//  UWB Spatial Tracking - TAG Firmware (SET 2, CHANNEL 9)
//
//  IDs:   anchors 6, 7, 8, 9 | tag 10 | listener = anchor 6
//  RF:    UWB channel 9
//
//  1 tag + 4 anchors (1 listener + 3 dumb), no TDMA. The tag
//  ranges with each anchor in a tight loop and broadcasts the
//  four distances to the listener (anchor 6) which streams
//  them as CSV over USB serial.
//
//  Cycle:
//    1. DS-TWR with anchor 6 (listener), then 7, 8, 9
//    2. Broadcast [tag_id, d6..d9, rssi6..rssi9] to anchor 6
//    3. Optional inter-cycle delay (CYCLE_DELAY_MS)
//
//  All three firmwares in this set (tag, listener, dumb) must
//  agree on channel and antenna delay. Set 1 (channel 5,
//  anchors 1-4, tag 5) runs concurrently on a different channel
...
#define FIRST_ANCHOR_ID     6
#define LISTENER_ANCHOR_ID  6
