...
void sendDataOverSerial()
{
    ...
    for (int i = 0; i < NUM_ANCHORS; i++)
    {
        ...
        Serial.print(anchors[i].filtered_distance, 2);
        Serial.print(",\"raw\":");
        Serial.print(anchors[i].distance, 2);
        Serial.print(",\"rssi\":");
        Serial.print(anchors[i].signal_strength, 2);
        Serial.print("}");
    }
    Serial.println("}}");
}
...
{
    uint32_t tmp = readOrWriteFullAddress(base, sub, 0, 0, 0);
